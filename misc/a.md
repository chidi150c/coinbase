You are updating a running production trading bridge. Safety and behavior-preservation are critical.

Task:
Generate a full copy of bridge_binance/ws_binance.py: {{#!/usr/bin/env python3 
...
}} with only the necessary minimal changes to implement:

add a best-effort retry wrapper and ≤2000ms success-only in-memory cache around the /api/v3/account call in bridge_binance/ws_binance.py for /accounts, /balance/base, and /balance/quote, without changing any JSON schemas, field names, numeric semantics, or other endpoints or trading logic

Hard constraints (DO NOT VIOLATE):

Do not alter any function names, metric names, environment variable keys, log message strings, or return values of identity-style functions.

Do not modify /price, /bbo, /candles, any /order* endpoints, or the _ws_loop websocket consumer in any way.

You may only change:

the /accounts, /balance/base, and /balance/quote handlers, and

small local helper(s) they directly use (e.g., a new _binance_accounts_cached() function and associated module-level cache variables).

Do not change the JSON schema, field names, or numeric semantics of any existing endpoint responses. The outputs of /accounts, /balance/base, and /balance/quote must be bit-for-bit compatible for the same underlying Binance response.

Retry + cache requirements:

Add a best-effort retry wrapper around the /api/v3/account call used by /accounts, /balance/base, and /balance/quote.

Use a small fixed retry count (e.g., 2 extra attempts) and a short sleep (e.g., 100–300 ms) between attempts.

Only catch transient network errors such as http.client.RemoteDisconnected, urllib.error.URLError, and OSError. Re-raise any error after retries so the current error behavior is preserved.

Add a very short-lived in-memory cache for /api/v3/account responses:

Cache only successful responses (HTTP 200 with valid JSON).

Cache TTL must be ≤ 2000 ms (2 seconds).

If the cache is empty or expired, call Binance live exactly as before and update the cache.

/accounts, /balance/base, and /balance/quote must read from this cache when valid, otherwise fall back to a live call.

Do not introduce any background tasks, timers, threads, or async jobs. The cache should be checked and updated synchronously inside the request handlers/helpers.

Style and scope:

Only apply the minimal edits required to implement the retry + cache behavior described above.

Do not refactor unrelated code, move functions around, or change logging formats.

Fix any syntax/compile errors you introduce, but do not “clean up” or reformat unrelated parts of the file.

Output:

Return the complete, copy-paste-ready contents of bridge_binance/ws_binance.py as a single code block, with all unchanged code preserved verbatim.