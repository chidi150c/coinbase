...
}} with only the necessary minimal changes to implement {{add persisted PendingBuy/PendingSell and pendingRecheck fields plus RehydratePending(ctx, RehydrateModeResume) in ./trader.go to resume post-only pending opens by restoring OrderID and remaining deadline and restarting the poller when GetOrder/ListOpenOrders confirm the order is still open, and call RehydratePending(ctx, RehydrateModeResume) in ./main.go}}. Do not alter any function names, struct names, metric names, environment keys, log strings, or the return value of identity functions (e.g., Name()). Keep all public behavior, identifiers, and monitoring outputs identical to the current baseline. Only apply the minimal edits required to implement {{add persisted PendingBuy/PendingSell and pendingRecheck fields plus RehydratePending(ctx, RehydrateModeResume) in ./trader.go to resume post-only pending opens by restoring OrderID and remaining deadline and restarting the poller when GetOrder/ListOpenOrders confirm the order is still open, and call RehydratePending(ctx, RehydrateModeResume) in ./main.go}}. Fixe any compile error(s). Return the complete file, copy-paste ready, in IDE.
