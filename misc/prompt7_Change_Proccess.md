...
const (
	EntrySourceNormalBuy  EntryProducer = "normal_buy"
	EntrySourceNormalSell EntryProducer = "normal_sell"
...
func (t *Trader) drainPendingCase3AEntries(
	...
	/*
		...
		entry from pendingCase3A. A snapshot keeps iteration independent
...
] Generate a full copy of each one of these functions if you required any change in it with only the necessary minimal changes to implement {{ add entry commit behavior policy that controls commit side effects based on the pending entry source }}. Do not alter any function names, struct names, metric names, environment keys, log strings, or the return value of identity functions (e.g., Name()). Keep all public behavior, identifiers, and monitoring outputs identical to the current baseline. Only apply the minimal edits required to implement {{add entry commit behavior policy that controls commit side effects based on the pending entry source}}. Fix any compile error(s). Return the complete files (new of modified existing ones), copy-paste ready, in IDE.
