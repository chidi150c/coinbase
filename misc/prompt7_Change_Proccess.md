...
}} with only the necessary minimal changes to implement {{the above prompt}}. Do not alter any function names, struct names, metric names, environment keys, log strings, or the return value of identity functions (e.g., Name()). Keep all public behavior, identifiers, and monitoring outputs identical to the current baseline. Only apply the minimal edits required to implement {{the above prompt}}. Return the complete file, copy-paste ready, in IDE.
