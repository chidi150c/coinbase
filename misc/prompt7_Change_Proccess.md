...
}} with only the necessary minimal changes to implement {{replace baseStepOverride(), quoteStepOverride(), and requireBaseForShort() usages with t.cfg.BaseStep, t.cfg.QuoteStep, and t.cfg.RequireBaseForShort

snap limit/TP prices to t.cfg.PriceTick when >0

prefer t.cfg.MinNotional over ORDER_MIN_USD for min-notional checks (fall back to ORDER_MIN_USD if MinNotional ≤ 0)

add entryTake local variable and assign it to newLot.Take while removing undefined stop/take logs

add stable fallback when step values are zero (quote_step=0.01 for stablecoins; base_step=1e-8) with WARN logs}}. Do not alter any function names, struct names, metric names, environment keys, log strings, or the return value of identity functions (e.g., Name()). Keep all public behavior, identifiers, and monitoring outputs identical to the current baseline. Only apply the minimal edits required to implement {{replace baseStepOverride(), quoteStepOverride(), and requireBaseForShort() usages with t.cfg.BaseStep, t.cfg.QuoteStep, and t.cfg.RequireBaseForShort
...
add stable fallback when step values are zero (quote_step=0.01 for stablecoins; base_step=1e-8) with WARN logs}}. Return the complete file, copy-paste ready, in IDE.
