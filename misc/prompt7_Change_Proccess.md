...
}} with only the necessary minimal changes to implement {{refactor Trader mutex usage by moving all broker I/O and saveState calls outside locks, centralizing state mutations in a single manager goroutine using RWMutex-backed snapshots, immediately updating and persisting Pending{Buy,Sell}.OrderID on each reprice, and adding defer-based Unlock with panic-safe logging}}. Do not alter any function names, struct names, metric names, environment keys, log strings, or the return value of identity functions (e.g., Name()). Keep all public behavior, identifiers, and monitoring outputs identical to the current baseline. Only apply the minimal edits required to implement {{refactor Trader mutex usage by moving all broker I/O and saveState calls outside locks, centralizing state mutations in a single manager goroutine using RWMutex-backed snapshots, immediately updating and persisting Pending{Buy,Sell}.OrderID on each reprice, and adding defer-based Unlock with panic-safe logging}}. Fix any compile error(s). Return the complete file, copy-paste ready, in IDE.
