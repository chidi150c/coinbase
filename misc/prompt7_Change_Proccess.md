...
}} with only the necessary minimal changes to implement {{apply exchange LOT_SIZE.stepSize and PRICE_FILTER.tickSize to bridge order quantity and price before submission in broker_bridge.go}}. Do not alter any function names, struct names, metric names, environment keys, log strings, or the return value of identity functions (e.g., Name()). Keep all public behavior, identifiers, and monitoring outputs identical to the current baseline. Only apply the minimal edits required to implement {{apply exchange LOT_SIZE.stepSize and PRICE_FILTER.tickSize to bridge order quantity and price before submission in broker_bridge.go}}. Return the complete file, copy-paste ready, in IDE.
